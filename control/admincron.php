...
$ressa = mysql_query("SELECT * FROM user WHERE acctype!=1 && upgrade_ends<='$the_day'") or die (mysql_error());
if (mysql_num_rows($ressa) != 0) {
 for ($vv = 0; $vv < mysql_num_rows($ressa); $vv++) {
  ...
  $doit = mysql_query("UPDATE user SET acctype=1, premregdate='', upgrade_ends='' WHERE id=$usrid");
...
@mysql_free_result($ressa);
if ($roi_conversion_ratio_free > 0 || $roi_conversion_ratio_pro > 0) {
 $res = mysql_query("SELECT * FROM user WHERE invested>0 && lastroi<'$the_day' ORDER BY id") or die ("Line: 22" . mysql_error());
 if (mysql_num_rows($res) != 0) {
  for ($i = 0; $i < mysql_num_rows($res); $i++) {
   $theusr = mysql_result($res, $i, "id");
   $my_invest = mysql_result($res, $i, "invested");
   $my_acctype = mysql_result($res, $i, "acctype");
   if ($my_acctype == 1) {
    $roi_rate = round($roi_conversion_ratio_free / 100, 3);
    $roi_credit_return = $my_invest * $roi_rate;
    $get_stats = mysql_query("SELECT num FROM 7stat WHERE num>=$min_credits_to_earn_free && date='$the_day' && received_pay='no' && usrid=$theusr") or die ("Line: 31" .mysql_error());
   } else {
    $roi_rate = round($roi_conversion_ratio_pro / 100, 3);
    $roi_credit_return = $my_invest * $roi_rate;
    $get_stats = mysql_query("SELECT num FROM 7stat WHERE num>=$min_credits_to_earn_pro && date='$the_day' && received_pay='no' && usrid=$theusr") or die ("Line: 35" .mysql_error());
   }
   if (mysql_num_rows($get_stats) != 0) {
    ...
    if (mysql_num_rows($get_stats) != 0) {
     @mysql_query("UPDATE monthly_stats SET roi_earned=roi_earned+$roi_credit_return, days_paid_roi=days_paid_roi+1, tot_owed=tot_owed+$roi_credit_return, this_month='$the_day' WHERE usrid=$theusr && monthis=$monthis && yearis=$yearis") or die(mysql_error());
    } else {
     @mysql_query("INSERT INTO monthly_stats (usrid, days_paid_roi, roi_earned, tot_owed, monthis, yearis, this_month) VALUES ($theusr, 1, $roi_credit_return, $roi_credit_return, $monthis, $yearis, '$the_day')") or die(mysql_error());
    }
    $tot_paidto++;
   }
  }
